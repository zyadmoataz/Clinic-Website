...
- **Rule**: Never use Tailwind's `dark:` utility classes. Instead, use semantic variables:
  ...
  <!-- ❌ INCORRECT -->
  <div class="bg-white text-black dark:bg-slate-900 dark:text-white">Content</div>
  ```
...
- _Result_: Both developers work in isolation. Omar builds lists; Doaa designs forms with complete datasets.
...
- _Result_: Helda builds lists, status badges, and cancellation triggers without a single backend API call.
